...
class OBJModel extends Model {
    ...
    constructor(src, obj) {
        super(src, obj);
        obj = Check.checkInput(this.property, DefaultSetting_OBJ);
        this.property = obj;
    }
    ...
    load(name) {
        ...
        if (this.property.onload === undefined) {
            if (this.property.position.length > 3 || this.property.position.length < 2 || this.property.position.length === undefined) {
                console.info('Speed3D.OBJModel:请输入有效的模型的经纬度和高度');
                return;
            }
            ...
            this.property.onload = function (obj) {
                ...
                let a = obj.detail.loaderRootNode.children;
                let ids = [];
                let gap = 0;
                if (a.length > 1) {
        ...
        loader.load(that.src, that.property.onload, that.property.onProgress, that.property.onError, that.property.onMeshAlter, that.property.useAsync);
    }
}
